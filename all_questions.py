...
def question2():
    ...
    # Answers are floats
    answer["(a) entropy_entire_data"] = 1.422
    # Infogain
    answer["(b) x <= 0.2"] = 1.266
    answer["(b) x <= 0.7"] = 0.926
    answer["(b) y <= 0.6"] = 0.767
...
def question6():
    ...
    answer["a, level 1"] = "x <= c"
    answer["a, level 2, right"] ="y <= c"
    answer["a, level 2, left"] = "x <= c"
    answer["a, level 3, left"] = "y <= c2"
    answer["a, level 3, right"] = "x > c2"

    # run each datum through the tree. Count the number of errors and divide by number of samples. .
    # Since we have areas: calculate the area that is misclassified (total area is unity)
    # float between 0 and 1
    answer["b, expected error"] = 1.427

    # Use u.BinaryTree to define the tree. Create your tree.
    # Replace "root node" by the proper node of the form "z <= float"
    tree = u.BinaryTree("x <= c")
    A = tree.insert_left("x <= c")
    B = tree.insert_right("y <= c")
    A.insert_left("A")
    A.insert_right("B")
    B.insert_left("A")
    B.insert_right("A")
